...
export default function ProfilePage() {
      ...
      try {
        ...
        if (targetUid) {
          try {
            ...
            // Defer session details — don't block initial render
            // Load details in the background after the page is visible
            setLoading(false)
            if (sessionList.length > 0) {
              const detailPromises = sessionList.slice(0, 5).map(s => getSession(s.id))
  ...
  const getShareUrl = useCallback(() => {
    ...
    return `${window.location.origin}/profile/${publicProfile.username}`
  }, [publicProfile])
  ...
  const shareToWhatsApp = () => {
    const url = getShareUrl()
    const text = `Check out ${displayName}'s developer profile on CodePath AI`
  ...
  const shareViaEmail = () => {
    const url = getShareUrl()
    const subject = `${displayName}'s Developer Profile`
    const body = `Check out this developer profile on CodePath AI:\n\n${url}`
  ...
  const shareNative = async () => {
    const url = getShareUrl()
    try {
      await navigator.share({ title: `${displayName} — CodePath AI`, url })
  ...
  if (authLoading || loading) {
    return (
      <div className="min-h-screen bg-[#09090f] text-white/70 page-enter">
        {/* Skeleton header */}
        <header className="sticky top-0 z-40 bg-[#09090f]/80 backdrop-blur-md border-b border-white/[0.04]">
          <div className="max-w-3xl mx-auto px-6 py-4 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <div className="w-20 h-7 skeleton rounded-md" />
              <div className="w-24 h-7 skeleton rounded-md" />
            </div>
          </div>
        </header>
        {/* Skeleton body */}
          ...
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5 space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {[80, 60, 90, 50, 70].map((w, i) => (
                <div key={i} style={{ width: w }} className="h-6 skeleton rounded-full" />
              ))}
            </div>
          </div>
          {/* Activity skeleton */}
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5 space-y-3">
            ...
            <div className="grid grid-cols-7 gap-1">
              {Array.from({ length: 35 }).map((_, i) => (
                <div key={i} className="w-full aspect-square skeleton rounded-sm" />
              ))}
            </div>
          </div>
          {/* Sessions skeleton */}
        ...
        <div className="max-w-3xl mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative" ref={shareMenuRef}>
              ...
              {/* ── Share dropdown ── */}
              {showShareMenu && (
                <div className="absolute right-0 mt-2 w-52 rounded-xl bg-[#13131f] border border-white/[0.08] shadow-2xl shadow-black/60 z-50 py-1.5 animate-in fade-in slide-in-from-top-2 duration-150">
                  <button onClick={copyProfileUrl} className="w-full flex items-center gap-2.5 px-3.5 py-2 text-[12px] text-white/50 hover:text-white/80 hover:bg-white/[0.04] transition-colors">
                  ...
                  <button onClick={shareToWhatsApp} className="w-full flex items-center gap-2.5 px-3.5 py-2 text-[12px] text-white/50 hover:text-emerald-400 hover:bg-emerald-500/[0.06] transition-colors">
                  ...
                  <button onClick={shareToTwitter} className="w-full flex items-center gap-2.5 px-3.5 py-2 text-[12px] text-white/50 hover:text-sky-400 hover:bg-sky-500/[0.06] transition-colors">
                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                    X (Twitter)
                  </button>
                  <button onClick={shareToLinkedIn} className="w-full flex items-center gap-2.5 px-3.5 py-2 text-[12px] text-white/50 hover:text-blue-400 hover:bg-blue-500/[0.06] transition-colors">
                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                    LinkedIn
                  </button>
                  <button onClick={shareViaEmail} className="w-full flex items-center gap-2.5 px-3.5 py-2 text-[12px] text-white/50 hover:text-amber-400 hover:bg-amber-500/[0.06] transition-colors">
                  ...
                  {'share' in navigator && (
                    <>
                      ...
                      <button onClick={shareNative} className="w-full flex items-center gap-2.5 px-3.5 py-2 text-[12px] text-white/50 hover:text-violet-400 hover:bg-violet-500/[0.06] transition-colors">
        ...
        <section className="border-t border-white/[0.06] pt-6 pb-4 text-center space-y-3">
          ...
          <div className="flex items-center justify-center gap-3 pt-2">
            ...
            <button
              ...
              className="text-[11px] px-4 py-1.5 rounded-md bg-emerald-500/[0.06] text-emerald-300/50 border border-emerald-500/[0.12] hover:bg-emerald-500/[0.12] transition-colors"
            >
              Share on WhatsApp
            </button>
            <button
