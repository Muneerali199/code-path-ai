...
function extractContent(parsed: any): string | undefined {
    ...
    if (delta) {
        ...
        if (Array.isArray(delta?.content)) {
            const texts = delta.content.map((c: any) => c?.text || c?.content || '').filter(Boolean);
            if (texts.length > 0) return texts.join('');
        }
    }
...
function buildMessages(command: string, text: string, lang?: string, issue?: string, focus?: string): Message[] {
    switch (command) {
        case 'explain':
            return [{
                role: 'user',
...
async function sendChat(payload: ChatPayload, stream: boolean): Promise<string> {
...
async function runRepl(): Promise<void> {
    ...
    if (history.length > 0) {
        (rl as unknown as { history: string[] }).history = history.slice(-500).reverse();
    }
        ...
        if (input.startsWith('/')) {
            ...
            switch (cmd) {
                ...
                case '/model':
                    ...
                    return;
                case '/thinking':
                    if (parts[1] === 'on') thinking = true;
                    if (parts[1] === 'off') thinking = false;
                    console.log(`Thinking: ${thinking ? 'on' : 'off'}`);
                    prompt();
                    return;
                case '/stream':
                    if (parts[1] === 'on') useStream = true;
                    if (parts[1] === 'off') useStream = false;
